...
local function initObject(unit, style, styleFunc, header, ...)
	...
	for i = 1, num do
		...
		local Portraitfix = object:GetAttribute('unitPortraitfix')

		-- Handle the case where someone has modified the unitPortraitfix attribute in
		-- oUF-initialConfigFunction.
		if(Portraitfix and not objectUnit:match(Portraitfix)) then
			objectUnit = objectUnit .. Portraitfix
		end
...
local function generateName(unit, ...)
	...
	local raid, party, groupFilter, unitPortraitfix
	for i = 1, select('#', ...), 2 do
		local att, val = select(i, ...)
		if(att == 'oUF-initialConfigFunction') then
			unitPortraitfix = val:match('unitPortraitfix[%p%s]+(%a+)')
	...
	if(append) then
		name = name .. append .. (unitPortraitfix or '')
	end
	...
	local initialConfigFunction = [[
		...
		for i = 1, #frames do
			...
			if(not frame:GetAttribute('oUF-onlyProcessChildren')) then
				...
				local Portraitfix = frame:GetAttribute('unitPortraitfix')
				if(unit and Portraitfix) then
					if(headerType == 'pet' and Portraitfix == 'target') then
						unit = unit .. headerType .. Portraitfix
					else
						unit = unit .. Portraitfix
					end
