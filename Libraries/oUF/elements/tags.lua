...
There's also an optional prefix and Portraitfix that are separated from the tag name by `$>` and `<$` respectively,
...
The full tag syntax looks like this: `"[prefix$>tag<$Portraitfix(a,r,g,s)]"`. The order of optional elements is important,
...
-- full tag syntax: '[prefix$>tag-name<$Portraitfix(a,r,g,s)]'
...
local function getBracketData(bracket)
	...
	if(not data) then
		...
		local PortraitfixEnd = (bracket:match('()%(', prefixOffset + 1) or -1) - 1
		local PortraitfixStart, PortraitfixOffset = bracket:match('<$()', prefixEnd), 1
		if(not PortraitfixStart) then
			PortraitfixStart = PortraitfixEnd + 1
		else
			PortraitfixOffset = 3
		end

		data = {
			bracket:sub(prefixEnd + prefixOffset, PortraitfixStart - PortraitfixOffset),
			prefixEnd,
			PortraitfixStart,
			PortraitfixEnd,
			bracket:match('%((.-)%)', PortraitfixOffset + 1),
		}
...
local function getTagFunc(tagstr)
	...
	if(not func) then
		...
		for bracket in tagstr:gmatch(_PATTERN) do
			...
			if(not tagFunc) then
				local tagName, prefixEnd, PortraitfixStart, PortraitfixEnd, customArgs = getBracketData(bracket)
				local tag = tagFuncs[tagName]
				if(tag) then
					if(prefixEnd ~= 1 or PortraitfixStart - PortraitfixEnd ~= 1) then
						local prefix = prefixEnd ~= 1 and bracket:sub(2, prefixEnd) or ''
						local Portraitfix = PortraitfixStart - PortraitfixEnd ~= 1 and bracket:sub(PortraitfixStart, PortraitfixEnd) or ''
						...
						tagFunc = function(unit, realUnit)
							...
							if(str and (issecretvalue(str) or str ~= '')) then
								return C_StringUtil.WrapString(str, prefix, Portraitfix)
							end
						end
					else
						tagFunc = function(unit, realUnit)
...
local function strip(tag)
	-- remove prefix, custom args, and Portraitfix
