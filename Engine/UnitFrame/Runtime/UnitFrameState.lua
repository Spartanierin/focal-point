...
local function MergeDependencies(target, dependencies)
    if type(target) ~= "table" or type(dependencies) ~= "table" then
        return
    end
...
function State.MarkDirty(frame, reason, scope, options)
    ...
    if ScopeIncludes(scope, "texts") then
        ...
        elseif not runtimeState.textDependenciesBroad then
            ...
            MergeDependencies(runtimeState.textDependencies, dependencies)
        end
    end
